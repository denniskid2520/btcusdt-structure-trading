"""Quantify bear market impulse shorts with different trail configurations.

Mirror of quantify_bull.py: find breakdown short entries in bear periods,
simulate each trade with ATR7 + ACCEL 3x trail, compare configs.

Run with: PYTHONPATH=src python quantify_bear.py
"""
import sys
...
def get_daily_macd_at(ts):
    ...
    for i, t in enumerate(daily_ts):
        if t <= ts:
            ...
            break
    return d_macd[best_idx]


def get_weekly_hist_at(ts):
    best_idx = 0
...
def sim_short_trade(bars_4h, entry_idx, entry_price, atr_mult, accel_mult,
                    time_limit_bars=168):
    """Simulate a short trade with trailing stop and bear ACCEL zone."""
    ...
    for i in range(entry_idx + 1, min(entry_idx + time_limit_bars, len(bars_4h))):
        ...
        # Bear ACCEL: weekly hist <= 0 (death cross) AND daily MACD < 0
        w_h = get_weekly_hist_at(bar.timestamp)
        d_m = get_daily_macd_at(bar.timestamp)
        bear_accel = w_h <= 0 and d_m < 0
        ...
        if bar.low < trough:
            trough = bar.low
            trail_stop = trough + atr * effective_mult

        # Check stop hit
        if bar.high >= trail_stop:
            return trail_stop, i, 'trail_stop', trough, accel_bars_count

    # Time stop
    exit_i = min(entry_idx + time_limit_bars - 1, len(bars_4h) - 1)
    return bars_4h[exit_i].close, exit_i, 'time_stop', trough, accel_bars_count


def find_bear_entries(bars_4h, start, end, lookback=30):
    """Find breakdown entries: close breaks below lookback-bar low after rally."""
    entries = []
    ...
    for idx, (gi, bar) in enumerate(segment_indices):
        ...
        prev_lows = [bars_4h[gi - k].low for k in range(1, lookback + 1)]
        prev_low = min(prev_lows)

        if bar.close < prev_low and bars_4h[gi - 1].close >= prev_low:
            recent_high = max(bars_4h[gi - k].high for k in range(1, min(lookback, 60) + 1))
            rally_pct = (recent_high - prev_low) / prev_low * 100
            if rally_pct >= 3:
                entries.append({
                    'idx': gi,
                    'price': bar.close,
...
# === Bear market periods ===
bear_periods = [
    {'name': '2021-11~2022-01 (ATH崩盤)', 'start': dt(2021, 11, 1), 'end': dt(2022, 1, 31)},
    {'name': '2022-03~2022-06 (Luna/3AC)', 'start': dt(2022, 3, 1), 'end': dt(2022, 6, 30)},
    {'name': '2022-08~2022-11 (延續熊市)', 'start': dt(2022, 8, 1), 'end': dt(2022, 11, 30)},
    {'name': '2024-03~2024-07 (ETF後修正)', 'start': dt(2024, 3, 1), 'end': dt(2024, 7, 31)},
    {'name': '2025-10~2026-03 (熊市下跌)', 'start': dt(2025, 10, 1), 'end': dt(2026, 3, 31)},
]
...
for period in bear_periods:
    entries = find_bear_entries(bars_4h, period['start'], period['end'])

    print('=' * 110)
    print(f"  {period['name']}")
    print(f"  找到 {len(entries)} 個崩盤入場點")
    print('=' * 110)

    for e in entries:
        w_h = get_weekly_hist_at(e['date'])
        d_m = get_daily_macd_at(e['date'])
        bear_accel = w_h <= 0 and d_m < 0

        print(f"\n  入場: {e['date'].strftime('%Y-%m-%d %H:%M')} @ ${e['price']:,.0f}  "
              f"(跌破 ${e['prev_low']:,.0f}, 反彈 {e['rally']:.1f}%)")
        print(f"  MACD: W-hist={w_h:+,.0f}  D-MACD={d_m:+,.0f}  "
              f"Bear ACCEL: {'YES' if bear_accel else 'NO'}")
        print()

        header = (f"  {'Config':<22s} {'Exit Date':<12s} {'Exit $':>10s} "
                  f"{'PnL':>8s} {'Days':>6s} {'Trough':>10s} {'Reason':<12s} ACCEL")
        print(header)
        print(f"  {'-' * 100}")

        for cname, atr_m, accel_m in configs:
            exit_p, exit_i, reason, trough, ab = sim_short_trade(
                bars_4h, e['idx'], e['price'], atr_m, accel_m)
            pnl_pct = (e['price'] - exit_p) / e['price'] * 100
            days = (bars_4h[exit_i].timestamp - e['date']).total_seconds() / 86400
            trough_pct = (e['price'] - trough) / e['price'] * 100
            ...
            print(f"  {cname:<22s} {bars_4h[exit_i].timestamp.strftime('%Y-%m-%d'):<12s} "
                  ...
                  f"${trough:>9,.0f} ({trough_pct:>+5.1f}%) {reason:<12s} {ab:>4d}{marker}")

# === Grand Summary ===
print()
print('=' * 110)
print('  GRAND TOTAL (所有熊市脈衝做空交易)')
print('=' * 110)
header = (f"  {'Config':<22s} {'Trades':>6s} {'Total PnL':>10s} "
          f"{'Avg/Trade':>10s} {'Win Rate':>9s} {'Max Win':>9s} {'Max Loss':>9s} {'Sharpe':>7s}")
print(header)
print(f"  {'-' * 90}")
...
for cname, _, _ in configs:
    ...
    print(f"  {cname:<22s} {n:>6d} {total:>+9.1f}% {avg:>+9.1f}% "
          f"{wr:>7.0f}% {max_w:>+8.1f}% {max_l:>+8.1f}% {sharpe:>6.2f}")
