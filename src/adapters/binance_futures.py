...
class BinanceFuturesAdapter(MarketDataAdapter):
    """Fetches real klines from Binance Futures (USDT-M) public API."""
    ...
    def fetch_range(
        ...
        while cursor < end_ms:
            ...
            for row in raw:
                all_bars.append(self._parse(row))
    ...
    def fetch_multi(
        ...
        Args:
            symbol: e.g. "BTCUSDT"
            timeframes: dict of timeframe -> limit, e.g. {"4h": 500, "1d": 60}

        Returns:
            dict of timeframe -> list[MarketBar]
        """
    ...
    def fetch_price(self, symbol: str) -> float:
        ...
        url = f"{self.base_url}/fapi/v1/premiumIndex?{query}"
